...
public class CameraActivity extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        view = inflater.inflate(getResources().getIdentifier("camera_activity", "layout", appResourcesPackage),
                container, false);
        createCameraPreview();
        return view;
    }
                                ...
                                frameContainerLayout.setOnTouchListener(
                                        new View.OnTouchListener() {
                                            ...
                                            public boolean onTouch(View v, MotionEvent event) {
                                                FrameLayout.LayoutParams layoutParams = (FrameLayout.LayoutParams) frameContainerLayout
                                                        .getLayoutParams();

                                                boolean isSingleTapTouch = gestureDetector.onTouchEvent(event);
                                                int action = event.getAction();
                                                int eventCount = event.getPointerCount();
                                                Log.d(TAG, "onTouch event, action, count: " + event + ", " + action
                                                        + ", " + eventCount);
                                                if (eventCount > 1) {
                                                            ...
                                                            setFocusArea(
                                                                    ...
                                                                    new Camera.AutoFocusCallback() {
                                                                        public void onAutoFocus(boolean success,
                                                                                Camera camera) {
                                                                            if (success) {
                                                                                // A callback to JS might make sense
                                                                                // here.
                                                                            } else {
                                                                                Log.d(TAG, "onTouch:"
                                                                                        + " setFocusArea() did not suceed");
    ...
    private Camera.Size getOptimalPictureSize(
        ...
        for (int i = 0; i < supportedSizes.size(); i++) {
            ...
            double difference = Math
                    .abs(previewAspectRatio - ((double) supportedSize.width / (double) supportedSize.height));
                ...
                new Camera.PreviewCallback() {
                    ...
                    public void onPreviewFrame(byte[] bytes, Camera camera) {
                        try {
                            ...
                            YuvImage yuvImage = new YuvImage(bytes, parameters.getPreviewFormat(), rect.width(),
                                    rect.height(), null);
    ...
    public void startRecord(
        ...
        Log.d(TAG, "CameraPreview startRecord camera: " + camera + " width: " + width + ", height: " + height +" file: "+filePath
                + ", quality: " + quality);
        Activity activity = getActivity();
        ...
        if (withFlash) {
            cameraParams
                    .setFlashMode(withFlash ? Camera.Parameters.FLASH_MODE_TORCH : Camera.Parameters.FLASH_MODE_OFF);
        ...
        try {
            ...
            Log.d("TAG>>>",profile +filePath+mOrientationHint+maxDuration);
            mRecorder.prepare();
            Log.d(TAG, "Starting recording");
            mRecorder.start();
            eventListener.onStartRecordVideo();
        } catch (RuntimeException e) {
            Log.e(TAG, "RuntimeException during recording setup", e);
